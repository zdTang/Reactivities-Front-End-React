...
# 8-10 - Redirect after submission

1. Notice the Promise will return another promise
2. Be very careful of if condition. falsy or truth. how to control data flow
3. useHistory can let use to anywhere. once we finish an operation, we can use useHistory to redirect to the destination we wish.

# 8-11 - Moving the home page outside of nav

we don't want display NavBar at the begin.  
But because there is no Route for Navbar, it will alway display.  
so that we use create a Route and use "render"




# 10-10 - Using an interceptor to handle API error responses

add logic to interceptor  
Axios interceptor + toaster

# 10 -11 - Adding a not found component

use browserHistory object to redirect to where we want to go.

several questions:
1. how Axios know error happens from the Server? by Status code?
2. When Axios re-throw the error, who will receive it, the Browser?  
this chapter is very important. try to understand it. and watch it several times.


For 400-BadRequest, there are two scenarios. one is normal BadRequest, for this use case, we can just display the Error message. but for ValidationError, it also has an Array for those validation information. so that we need to parse it.


Add ValidationError component and display it once Error happens.

# 10-13 - Handling 500 errors on the client

has error at first, always complain "ServerError is not a component", cannot fix it ,just copy code from "ServerError" of source code. 